...
// https://github.com/PowerShell/PowerShell/blob/main/src/System.Management.Automation/engine/regex.cs
...
public sealed class WildcardPattern
{
    ...
    /// <summary>
    /// Escape special chars, except for those specified in <paramref name="charsNotToEscape"/>, in
    /// a string by replacing them with their escape codes.
    /// </summary>
    ...
    /// <returns>
    /// A string of characters with any metacharacters, except for those specified in <paramref
    /// name="charsNotToEscape"/>, converted to their escaped form.
    /// </returns>
...
#pragma warning restore 56506
    ...
    /// To override the default escape character, specify the <paramref name="escapeCharacter"/>
    /// value.
    /// </remarks>
...
internal abstract class WildcardPatternParser
{
    ...
    /// </summary>
    protected abstract void AppendCharacterRangeToBracketExpression(char startOfCharacterRange,
    ...
    /// </remarks>
    internal void AppendBracketExpression(string brackedExpressionContents,
                                          string bracketExpressionOperators, string pattern)
    {
    ...
    public static void Parse(WildcardPattern pattern, WildcardPatternParser parser)
    {
        ...
        foreach (char c in pattern.Pattern)
        {
            ...
            {
                if (c == ']' && !previousCharacterStartedBracketExpression &&
                    !previousCharacterIsAnEscape)
                {
                    ...
                    parser.AppendBracketExpression(characterRangeContents.ToString(),
                                                   characterRangeOperators.ToString(),
                                                   pattern.Pattern);
                ...
                else if (c != pattern.EscapeCharacter || previousCharacterIsAnEscape)
                {
                    ...
                    characterRangeOperators.Append(
                        (c == '-') && !previousCharacterIsAnEscape ? '-' : ' ');
                }
            ...
            previousCharacterIsAnEscape =
                (c == pattern.EscapeCharacter) && (!previousCharacterIsAnEscape);
        }
        ...
        if (previousCharacterIsAnEscape)
        {
            if (!pattern.Pattern.Equals(
                    $"{pattern.EscapeCharacter}",
                    StringComparison
                        .Ordinal))  // Win7 backcompatibility requires treating '`' pattern as ''
                                    // pattern when this code was used with PowerShell.
            {
    ...
    {
        return new Exception($"The wildcard pattern, '{invalidPattern}', is invalid.");
    }
};
...
internal class WildcardPatternToRegexParser : WildcardPatternParser
{
    ...
    internal static void AppendLiteralCharacterToBracketExpression(StringBuilder regexPattern,
                                                                   char c)
    {
        if (c == '[')
        {
    ...
    internal static void AppendCharacterRangeToBracketExpression(StringBuilder regexPattern,
    ...
    protected override void AppendCharacterRangeToBracketExpression(char startOfCharacterRange,
                                                                    char endOfCharacterRange)
    {
        AppendCharacterRangeToBracketExpression(_regexPattern, startOfCharacterRange,
                                                endOfCharacterRange);
    }
...
internal class WildcardPatternMatcher
{
    ...
    internal bool IsMatch(string str)
    {
        ...
        for (int currentStringPosition = 0; currentStringPosition < str.Length;
             currentStringPosition++)
        {
            char currentStringCharacter =
                _characterNormalizer.Normalize(str[currentStringPosition]);
            ...
            while (patternPositionsForCurrentStringPosition.MoveNext(out int patternPosition))
            {
                _patternElements[patternPosition].ProcessStringCharacter(
                    currentStringCharacter, patternPosition,
        ...
        while (patternPositionsForCurrentStringPosition.MoveNext(out int patternPosition2))
        {
            _patternElements[patternPosition2].ProcessEndOfString(
                patternPosition2, patternPositionsForCurrentStringPosition);
        }
    ...
    private class PatternPositionsVisitor
    {
        ...
        public PatternPositionsVisitor(int lengthOfPattern)
        {
            ...
            {
                throw new ArgumentException($"Caller should verify {nameof(lengthOfPattern)} >= 0",
                                            nameof(lengthOfPattern));
            }
        ...
        public void Add(int patternPosition)
        {
            ...
            if (patternPosition < _lengthOfPattern)
            {
                _patternPositionsForFurtherProcessing[_patternPositionsForFurtherProcessingCount] =
                    patternPosition;
    ...
    private abstract class PatternElement
    {
        public abstract void ProcessStringCharacter(
            char currentStringCharacter, int currentPatternPosition,
    ...
    private class QuestionMarkElement : PatternElement
    {
        ...
        public override void ProcessEndOfString(
            ...
            PatternPositionsVisitor patternPositionsForEndOfStringPosition)
        {
            // '?' : (patternPosition, endOfString) => <no transitions out of this state - cannot
            // move beyond end of string>
        }
    }
    ...
    private class MyWildcardPatternParser : WildcardPatternParser
    {
        ...
        public static PatternElement[] Parse(WildcardPattern pattern,
                                             CharacterNormalizer characterNormalizer)
        {
            var parser = new MyWildcardPatternParser {
                _characterNormalizer = characterNormalizer,
                _regexOptions =
                    WildcardPatternToRegexParser.TranslateWildcardOptionsIntoRegexOptions(
                        pattern.Options),
            };
                                                                        ...
                                                                        char endOfCharacterRange)
        {
            WildcardPatternToRegexParser.AppendCharacterRangeToBracketExpression(
                _bracketExpressionBuilder, startOfCharacterRange, endOfCharacterRange);
        }
